...
void GetADValue( void )
{
    ...
    //计算内部参考电(1.224V)
    TempAD =  ADC_Value[7] ;
    if( IRV_ADValue > TempAD )
    {
        LimitTemp = IRV_ADValue - TempAD ;
    }
    else
    {
        LimitTemp = TempAD - IRV_ADValue ;
    }
    ...
    if( ( IRV_ADValue > 1719 ) || ( IRV_ADValue < 1319 ) )
    {
        IRV_ADValue  = 1519 ;
    }
    //计算出CJ431的比例系数
    TempAD =  ADC_Value[6] ;
    if( ADJ_ADValue > TempAD )
    {
        LimitTemp = ADJ_ADValue - TempAD ;
    }
    else
    {
        LimitTemp = TempAD - ADJ_ADValue ;
    }
    if( LimitTemp > 30 ) //变化超过一定范围，立即响应
    {
        ADJ_ADValue = TempAD ;
    ...
    if( TempAD < 3002 )
    {
        ISCJ431OKFlg = false ;    //因为参考电压AD值后面用于作为被除数(不能等于零,否则程序跑飞)
        ADJ_ADValue = 3102 ;
    }
    else if( TempAD > 3202 )
    {
    ...
    if( Volt_ADValue > TempAD )
    {
        LimitTemp = Volt_ADValue - TempAD ;
    }
    else
    {
        LimitTemp = TempAD - Volt_ADValue ;
    }
    if( LimitTemp > 30 )
    {
        Volt_ADValue = TempAD ;
    ...
    if( Sampling_Data[RUDDER] > TempAD )
    {
        LimitTemp = Sampling_Data[RUDDER] - TempAD  ;
    }
    else
    {
        LimitTemp = TempAD - Sampling_Data[RUDDER]  ;
    }
    if( LimitTemp < 20 )
    {
    ...
    if( TempAD > Input_Max )
    {
        Sampling_Data[RUDDER]   = Input_Max ;
    }
    else
    {
        Sampling_Data[RUDDER]   = TempAD ;
    }
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if( Sampling_Data[THROTTLE] > TempAD )
    {
        LimitTemp = Sampling_Data[THROTTLE] - TempAD ;
    }
    else
    {
        LimitTemp = TempAD - Sampling_Data[THROTTLE] ;
    }
    if( LimitTemp < 20 )
    {
    ...
    if( TempAD > Input_Max )
    {
        Sampling_Data[THROTTLE]   = Input_Max ;
    }
    else
    {
        Sampling_Data[THROTTLE]   = TempAD ;
    }
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //!!!!!  注意 : 美国手和日本手 THROTTLE 和 ELEVATOR 互换   !!!!!
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if( Sampling_Data[ELEVATOR] > TempAD )
    {
        LimitTemp = Sampling_Data[ELEVATOR] - TempAD ;
    }
    else
    {
        LimitTemp = TempAD - Sampling_Data[ELEVATOR] ;
    }
    if( LimitTemp < 20 )
    {
    ...
    if( TempAD > Input_Max )
    {
        Sampling_Data[ELEVATOR]   = Input_Max ;
    }
    else
    {
        Sampling_Data[ELEVATOR]   = TempAD ;
    }
    //计算AILERON
    TempAD =  ADC_Value[4] ;
    if( ISCJ431OKFlg == true )
    ...
    if( Sampling_Data[AILERON] > TempAD )
    {
        LimitTemp = Sampling_Data[AILERON] - TempAD ;
    }
    else
    {
