...
static void FT_GetHSKMidValue( void )
{
    ...
    else
    {
        ...
        for( i = 0; i < 10 ; i++ )
        {
            if( MaxValue_Temp < SamplingBuffer[RUDDER][i] )
            {
                MaxValue_Temp = SamplingBuffer[RUDDER][i] ;
            }
            if( MinValue_Temp > SamplingBuffer[RUDDER][i] )
            {
                MinValue_Temp = SamplingBuffer[RUDDER][i] ;
            }
        ...
        if( ( Temp < AD_MidValue_Min ) || ( Temp > AD_MidValue_Max ) )   //中位值校准有效性判断
        {
            ...
            LED_State_Shake |= LED_CH5 ;
            LED_State_ON &= ~LED_CH5 ;  // RUDDER 中位值 校准故障提示  CH5_L LED闪烁
        }
        else
        {
            LED_State_ON |= LED_CH5 ;
            LED_State_Shake &= ~LED_CH5 ;  // RUDDER 中位值 校准正常提示
        ...
        for( i = 0; i < 10 ; i++ )
        {
            if( MaxValue_Temp < SamplingBuffer[ELEVATOR][i] )
            {
                MaxValue_Temp = SamplingBuffer[ELEVATOR][i] ;
            }
            if( MinValue_Temp > SamplingBuffer[ELEVATOR][i] )
            {
                MinValue_Temp = SamplingBuffer[ELEVATOR][i] ;
            }
        ...
        {
            FTDebug_err_flg = true ;
            LED_State_Shake |= LED_CH6_H ;
            LED_State_ON &= ~LED_CH6_H ;  // ELEVATOR 中位值 校准故障提示
        }
        else
        {
            LED_State_ON |= LED_CH6_H ;
            LED_State_Shake &= ~LED_CH6_H ;  // ELEVATOR 中位值 校准正常提示
        ...
        for( i = 0; i < 10 ; i++ )
        {
            if( MaxValue_Temp < SamplingBuffer[AILERON][i] )
            {
                MaxValue_Temp = SamplingBuffer[AILERON][i] ;
            }
            if( MinValue_Temp > SamplingBuffer[AILERON][i] )
            {
                MinValue_Temp = SamplingBuffer[AILERON][i] ;
            }
        ...
        {
            FTDebug_err_flg = true ;
            LED_State_Shake |= LED_CH7 ;
            LED_State_ON &= ~LED_CH7 ;  // AILERON 中位值 校准故障提示
        }
        else
        {
            LED_State_ON |= LED_CH7 ;
            LED_State_Shake &= ~LED_CH7 ;  // AILERON 中位值 校准正常提示
        ...
        if( FTDebug_err_flg == false )
        {
            ...
            {
                beepCmd( NormalFreCounts, __stMidADAdjustDown );
            }
        }
        else
        {
...
{
    if( Sampling_OK_DelayCNT < 1000 )
    {
        ++Sampling_OK_DelayCNT;
    }
    else
    {
        Sampling_OK_DelayCNT = 0 ;
...
static void  FT_GetHSKMaxMinValue( void )
{
    ...
    else if( DisLEDCnts < 200 )
    {
        LED_State_ON = LED_CH6_M ;
    }
    else if( DisLEDCnts < 300 )
    {
        LED_State_ON = LED_CH5 ;
    }
    else if( DisLEDCnts < 400 )
    {
        LED_State_ON = LED_CH7 ;
    }
    else if( DisLEDCnts < 500 )
    {
    ...
    if( MAXValueDebug_RightOrLeftflg == false )
    {
        ...
        {
            if( Sampling_MaxMinData[RUDDER][MAXDAT] < Sampling_Data[RUDDER] )
            {
                Sampling_MaxMinData[RUDDER][MAXDAT] = Sampling_Data[RUDDER] ;
                SamplingMaxMinValue_StabilizeCnts = 0 ;
            }
            if( Sampling_MaxMinData[RUDDER][MINDAT] > Sampling_Data[RUDDER] )
            {
                Sampling_MaxMinData[RUDDER][MINDAT] = Sampling_Data[RUDDER] ;
                SamplingMaxMinValue_StabilizeCnts = 0 ;
            }
            if( Sampling_MaxMinData[THROTTLE][MAXDAT] < Sampling_Data[THROTTLE] )
            {
                Sampling_MaxMinData[THROTTLE][MAXDAT] = Sampling_Data[THROTTLE] ;
                SamplingMaxMinValue_StabilizeCnts = 0 ;
            }
            if( Sampling_MaxMinData[THROTTLE][MINDAT] > Sampling_Data[THROTTLE] )
            {
                Sampling_MaxMinData[THROTTLE][MINDAT] = Sampling_Data[THROTTLE] ;
                SamplingMaxMinValue_StabilizeCnts = 0 ;
            }
            ...
            {
                if( SamplingMaxMinValue_StabilizeCnts < 1000 )
                {
                    ++SamplingMaxMinValue_StabilizeCnts ;
                }
                else
                {
        ...
        else
        {
            ...
            if( Sampling_MaxMinData[ELEVATOR][MAXDAT] < Sampling_Data[ELEVATOR] )
            {
                Sampling_MaxMinData[ELEVATOR][MAXDAT] = Sampling_Data[ELEVATOR] ;
                SamplingMaxMinValue_StabilizeCnts = 0 ;
            }
            if( Sampling_MaxMinData[ELEVATOR][MINDAT] > Sampling_Data[ELEVATOR] )
            {
                Sampling_MaxMinData[ELEVATOR][MINDAT] = Sampling_Data[ELEVATOR] ;
                SamplingMaxMinValue_StabilizeCnts = 0 ;
            }
            if( ( Sampling_MaxMinData[RUDDER][MAXDAT]   > MAXValue_Min ) && ( Sampling_MaxMinData[RUDDER][MINDAT]   < MINValue_Max )
    ...
    else
    {
        ...
        if( RFHabit == __AmericaPlayer )
        {
            if( Sampling_MaxMinData[AILERON][MAXDAT] < Sampling_Data[AILERON] )
            {
                Sampling_MaxMinData[AILERON][MAXDAT] = Sampling_Data[AILERON] ;
                SamplingMaxMinValue_StabilizeCnts = 0 ;
            }
            if( Sampling_MaxMinData[AILERON][MINDAT] > Sampling_Data[AILERON] )
            {
                Sampling_MaxMinData[AILERON][MINDAT] = Sampling_Data[AILERON] ;
                SamplingMaxMinValue_StabilizeCnts = 0 ;
            }
            if( Sampling_MaxMinData[ELEVATOR][MAXDAT] < Sampling_Data[ELEVATOR] )
            {
