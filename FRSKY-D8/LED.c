...
void LED_Usart1Send( void )
{
    ...
    while( !( USART1 -> SR & ( 1 << 7 ) ) );
    USART_SendData8( USART1, LED_Status_SendDat );
    while( !( USART1 -> SR & ( 1 << 7 ) ) );
    USART_SendData8( USART1, ~LED_Status_SendDat );
}
...
void Volt_LED( float Volt_Value )
{
    ...
    if( VoltLED_ShakeRateCnt < VoltLED_Counts )
    {
        ++VoltLED_ShakeRateCnt ;
    }
    else
    {
        VoltLED_ShakeRateCnt = 0 ;
    }
