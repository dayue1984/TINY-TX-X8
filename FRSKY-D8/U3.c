...
void BuildUsart3Data( void )
{
    ...
    switch( Phase )
    {
        case 0:
            //                  L                         D                          A                         T                         X                          1                        0
            U3_DMA_TXBuff[0]  = 0x4C ;
            U3_DMA_TXBuff[1] = 0X44 ;
            U3_DMA_TXBuff[2] = 0x41 ;
            U3_DMA_TXBuff[3] = 0x54 ;
            U3_DMA_TXBuff[4] = 0X58 ;
            U3_DMA_TXBuff[5] = 0x31 ;
            U3_DMA_TXBuff[6] = 0x30 ; //起始头(7Byte)
            U3_DMA_TXBuff[7]  = RF_TypeVersion ;                                            //遥控器类型(FRSKYD8)
            U3_DMA_TXBuff[8]  = 18   ;
            U3_DMA_TXBuff[9] =  1 ;
            U3_DMA_TXBuff[10] = 25 ;                         //软件编译时间 : 2018 - 1 - 25
        ...
        case 4:
            ...
            {
                if( i < TRANSMITTER_CHANNEL )
                {
                    DataTemp = FRSKYD8_SendDataBuff[i];
                }
                else
                {
                    DataTemp = 0;
        ...
        case 7 :
            ...
            //20180801:AUX13、24分别参照DIP56，保留DIP78作为AUX短按长按选择
            if( GPIOF -> IDR & ( 1 << 0 ) )
            {
                DataTemp |= 0x08 ;    //CH5
                DataTemp |= 0x02 ;    //CH7
            ...
            if( CommunicationError_flg == false )
            {
                DataTemp |= ( 1 << 1 ) ;
            }
